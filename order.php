...
$object->query = "
    ...
    " . $where_condition . "
    ORDER BY order_type  DESC , order_id DESC
";
...
if (isset($_POST["add_order"])) {
    ...
    $formdata['order_type'] = $_POST['order_type'];
    $formdata['discount_given'] = $_POST['discount_given'];
    ...
    if ($error == '') {
        $data = array(
            ...
            ':discount_given' => $formdata['discount_given']
        );

        $object->query = "
        INSERT INTO order_msbs 
        (patient_name, doctor_name, order_total_amount, order_created_by, order_status, order_added_on, order_updated_on,order_type,discount_given) 
        VALUES (:patient_name, :doctor_name, :order_total_amount, :order_created_by, :order_status, :order_added_on, :order_updated_on,:order_type,:discount_given)
        ";
    ...
    if (isset($_GET["action"], $_GET["code"])) {
        if ($_GET["action"] == 'add') {
            ...
            <form action="order.php">
                ...
                <input type="hidden" name="code" value="<?php echo $object->convert_data('add'); ?>" />

                <div class="form-check form-switch m-2">
                    <input  <?php if (isset($_GET['order_type'])) echo 'checked'; ?>  onChange="this.form.submit()" class="form-check-input" name="order_type" value="Whole" type="checkbox" role="switch" id="whole_sale_button">
                    <label class="form-check-label" for="flexSwitchCheckDefault">Whole Sale Bill</label>
                </div>
                ...
                <div class="card-body">
                    <form method="post">
                            ...
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <?php
                                        if (isset($_GET['order_type']) && $_GET['order_type'] == 'Whole') {
                                            ?>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td  ><span id="order_total_amount"><?php if (isset($_POST["order_total_amount"])) echo $_POST["order_total_amount"]; ?></span> </td>
                                        <td>18% GST</td>
                                        <td>
                                            <small>Discount</small>
                                            <input onkeyup="calculate_total()" value="0" onblur="calculate_total()" placeholder="Discount" type="number" name='discount_given' id="discount_given" class='form-control' /></td>
                                        <td  class="font-weight-bold"> <b> <span id="final_total_ammount"></span> </b> </td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                        ...
                        <div class="mt-4 mb-0">
                            ...
                            <input type='hidden' name='order_type' value='<?= isset($_GET['order_type']) ? 'Whole' : 'Retail' ?>' />
            ...
            <script>
                    ...
                    } else
                    {
                        ...
                        fetch('action.php', {
                            ...
                            } else
                            {
                                ...
                                var html = '<tr id="' + no + '">';
                                if (!document.querySelector("#whole_sale_button").checked) {
                ...
                function calculate_total()
                {
                    ...
                    let discount_given = _("discount_given").value
                    var total = 0;
                    if (qty.length > 0)
                    {
                    ...
                    _('hidden_order_total_amount').value = (((0.18 * total) + total) - ((0.18 * total) + total) * (discount_given / 100)).toFixed(2);
                    _('order_total_amount').innerHTML = total.toFixed(2);
                    _('final_total_ammount').innerHTML = (((0.18 * total) + total) - ((0.18 * total) + total) * (discount_given / 100)).toFixed(2) + ' RS'
            ...
            if ($order_id > 0) {
                ...
                foreach ($order_result as $order_row) {
                    ...
                    <div class="form-check form-switch m-2">
                        <input disabled  <?php if ($order_row['order_type'] == 'Whole') echo 'checked'; ?>  onChange="this.form.submit()" class="form-check-input" name="order_type" value="Whole" type="checkbox" role="switch" id="whole_sale_button">
                        <label class="form-check-label" for="flexSwitchCheckDefault">Whole Sale Bill</label>
                    </div>
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td  id="order_total_amount"><?php echo $order_row["order_total_amount"]; ?></td>
                                                <td>18% GST</td>
                                                <td>
                                                    <small>Discount</small>
                                                    <input value="<?= $order_row['discount_given'] ?>" onkeyup="calculate_total()" onblur="calculate_total()" placeholder="Discount" type="number" name='discount_given' id="discount_given" class='form-control' /></td>
                                                <td  class="font-weight-bold"> <b> <span id="final_total_ammount"><?php echo $order_row["order_total_amount"]; ?> RS</span> </b> </td>
